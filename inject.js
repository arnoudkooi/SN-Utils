...
function snuAddTechnicalNames() {
    ...
    if (typeof g_form != 'undefined') {
        var snuVarLinkTable = null;
        var snuFirstSysId = null;
        g_form.nameMap.each(v => { if (!snuFirstSysId) snuFirstSysId = v.realName.substr(v.realName.length - 32); });
        if (snuFirstSysId) {
            try {
                var xhr = new XMLHttpRequest();
                xhr.open('GET', '/api/now/table/item_option_new/' + snuFirstSysId + '?sysparm_fields=sys_id', false);
                xhr.setRequestHeader('Accept', 'application/json');
                xhr.setRequestHeader('X-UserToken', window.g_ck);
                xhr.send();
                snuVarLinkTable = xhr.status === 200 ? 'item_option_new' : 'sc_item_option';
            } catch (e) {
                snuVarLinkTable = 'sc_item_option';
            }
        }
        g_form.nameMap.each(vari => {
            ...
            if (elm && !elm.classList.contains('snutn')) {
                try {
                    ...
                    var linkTableName = snuVarLinkTable || 'item_option_new';
